...
public enum GADAdTYPET
{
    case banner_Native1
    case full_Native1
    case adptive_Banner1
    case custom_Native
}
...
extension GoogleAd_Manager
{
    ...
    public func funShowNativeAd(adType : GADAdTYPET, parentView: UIView, shimerView : Shimmer_View? = nil, nativeAdView: GADNativeAdView? = nil, isAdShown : @escaping ((Bool) -> Void), isClick : @escaping (() -> Void), isNewLoad : @escaping (() -> Void))
    {
        var adtype = adType
...
extension UIView
{
    fileprivate func setup_NativeAdView(adType : GADAdTYPET,_ isAdShown : (() -> Void))
    {
        let tempNIBName = adType == .banner_Native1 ? "NativeBannerAd" : "NativeAd"
        ...
        self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[_nativeAdView]|",
                                                                options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: viewDictionary))
        if adType == .banner_Native1 {
            adView.set_NativeBanner()
            isAdShown()
        }
        else if adType == .full_Native1 {
